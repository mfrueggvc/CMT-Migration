...
% Resolve project root from this file location: .../matlab/plots/run_plots.m
...
if ~isfile(file1_name) || ~isfile(file2_name)
    error("Files not found. Expected:\n%s\n%s", file1_name, file2_name);
end

outputFolder = fullfile(comparisonDir, plotsSubfolder);
...
for i = 1:numel(uniqueCountries)
    ...
    idx = (T2.Country == currentCountry);
    countryData = T2(idx, :);
    [~, sortIdx] = sort(countryData.Year);
    countryData = countryData(sortIdx, :);
    
    % Create figure (Invisible to save speed)
    f_ts = figure('Name', ['Time Series: ' char(currentCountry)], 'Color', 'w', 'Visible', 'off');
    sgtitle(['Time Series Analysis: ' char(currentCountry)]);
    ...
    % Save
    cleanCountryName = replace(currentCountry, " ", "_"); 
