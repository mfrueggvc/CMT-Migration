...
fprintf('Updates: Model upgraded to Stepwise Regression (BIC) + C Comparator Integrated\n');
fprintf('Config: All files (Script, CSVs, C Source) assumed in the same directory.\n\n'); 

thisFile    = mfilename('fullpath');        
baseDir     = fileparts(thisFile);         % Current directory of the script

% PATHS ADJUSTED: All inputs in baseDir
dataDir     = baseDir;                     
resultsDir  = fullfile(baseDir, 'results'); 

% Define paths for C Comparator
% Define paths for C Comparator
cSourceFile = 'migration_compare_backup.c';
cSourcePath = fullfile(baseDir, cSourceFile); % Look in current directory
exePath     = fullfile(resultsDir, 'migration_compare_backup');

% Input paths for Comparator (Full paths required for C arguments)
truthPath   = fullfile(dataDir, 'NetMigration_SouthAmerica_1990_2019.csv');
predPath    = fullfile(resultsDir, 'predicted_migration_all_countries.csv');
...
%% C CODE COMPILE AND RUN
% Compile C Code
fprintf('\n=== Running C Comparator ===\n');
fprintf('Compiling %s...\n', cSourceFile);

% Define filename for output inside results dir
compOutName = 'comparison_output.csv';
compSumName = ['summary_' compOutName];

% Change directory to results so C code writes to valid path
oldDir = pwd;
cd(resultsDir);

% Relative path to executable in current dir
if ispc
    exeCmd = 'migration_compare_backup.exe';
else
    exeCmd = './migration_compare_backup';
end

% Go back to compile (assuming gcc needs full path or baseDir context)
cd(oldDir);

if ispc
    compileCmd = sprintf('gcc "%s" -o "%s.exe"', cSourcePath, exePath);
else
    compileCmd = sprintf('gcc "%s" -o "%s" -lm', cSourcePath, exePath); 
end
...
if status == 0
    ...
    fprintf('Running Comparator...\n');
    
    % Change to results dir to execute
    cd(resultsDir);
    
    % Execute: exe truth_path pred_path output_filename
    % Truth and Pred paths are absolute so they work from anywhere
    runCmd = sprintf('"%s" "%s" "%s" "%s"', exeCmd, truthPath, predPath, compOutName);
    
    [runStatus, runOutput] = system(runCmd);
    ...
    if runStatus == 0
        fprintf('%s\n', runOutput);
        fprintf('Comparator finished. Outputs located in:\n');
        fprintf('  %s\n', resultsDir);
        fprintf('  - %s\n', compOutName);
        fprintf('  - %s (Auto-generated by C code)\n', compSumName);
    else
        fprintf(' Runtime Error:\n%s\n', runOutput);
    end
else
    fprintf('Compilation Failed:\n%s\n', cmdout);
    fprintf('Ensure "gcc" is installed and in your system PATH.\n');
    fprintf('Ensure source file exists at: %s\n', cSourcePath);
end
