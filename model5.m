...
% Load Net Migration (later used only for training, up to 2014) 
...
% GDP: Linear interpolation with "extrapolation" 
...
% SchoolYears: "Monotonic smooth growth (pchip)" 
...
%  GDP Growth (year-over-year percentage) 
...
%  LOGARITHMIC transformations (diminishing returns effect) 
...
%  Unemployment-GDP interaction (economic stress indicator) 
...
% Replace any non-finite engineered features with zeros 
engineeredVars = {'GDPgrowth','GDPaccel','UnempChange','MigrationVelocity','OpportunityIndex','EconomicStress','TimeTrend','LogGDP','LogSchool'}; 
...
% Prediction: 2015-2019 (all data are used other than NetMigration) 
...
% Include unemployment and its interactions 
predVars = {'GDPgrowth', 'GDPaccel', 'LogGDP', 'LogSchool', ... 
'Unemployment', 'UnempChange', 'OpportunityIndex', ... 
'EconomicStress', 'MigrationLag1', 'MigrationLag2', ... 
'MigrationVelocity', 'TimeTrend'}; 
% Remove training rows with NaN 
...
'Unemployment + UnempChange + OpportunityIndex + EconomicStress + ' ... 
'MigrationLag1 + MigrationLag2 + MigrationVelocity + TimeTrend + ' ... 
'GDPgrowth:Unemployment + MigrationLag1:GDPgrowth + LogGDP:LogSchool']); 
...
% continent-wise data (sum up of all countries)
