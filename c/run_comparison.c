...
int load_truth_wide(const char *filename, Row rows[], int max_rows) {
    ...
    if (fgets(line, sizeof(line), fp)) {
        ...
        while ((token = strtok(NULL, ",;")) != NULL && num_countries < MAX_COLS) {
    ...
    while (fgets(line, sizeof(line), fp) && row_count < max_rows) {
        ...
        if (!token) continue;
        int year = atoi(token);
        for (int i = 0; i < num_countries && row_count < max_rows; i++) {
            token = strtok(NULL, ",;");
            if (token) {
                strip_clean(token);
...
double find_value(Row rows[], int n, const char *country, int year) {
    for (int i = 0; i < n; i++) {
        if (rows[i].year == year && strcmp(rows[i].country, country) == 0) return rows[i].value;
    }
    return 1e18; 
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < n_pred; i++) {
        ...
        if (actual < 1e17) {
            ...
            double pct_err = (fabs(actual) > 1e-9) ? (pred_rows[i].value - actual) / actual * 100.0 : 0.0;
            global_sum_ape += fabs(pct_err);
            ...
            country_stats[idx].sum_sq_error += fabs(pct_err); 
    ...
    for(int i=0; i<num_countries; i++) {
        ...
        if(s->count > 0) {
            ...
            if (s->count % 2 == 0) median_ae = (s->errors[s->count/2 - 1] + s->errors[s->count/2]) / 2.0;
            else median_ae = s->errors[s->count/2];
